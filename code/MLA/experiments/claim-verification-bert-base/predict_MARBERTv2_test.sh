...
out_dir="MARBERTv2-${max_len}-out_4neg_lr${lr}_5epochs_fold${fold}"
...
python '../../preprocess_claim_verification_AuRED.py' \
  ...
  --in_file "${pred_sent_dir}/MLA_test_4neg_lr${pred_lr}_5epochs_fold${fold}.jsonl" \
 ...
 python '../../postprocess_claim_verification_AuRED.py' \
  --data_file "${data_dir}/AuRED.json" \
  --fold_file "${data_dir}/training_folds/test_fold${fold}.tsv" \
  --pred_sent_file "${pred_sent_dir}/MLA_test_4neg_lr${pred_lr}_5epochs_fold${fold}.jsonl" \

...
class BaseTransformer(pl.LightningModule):
    ...
    def _feature_file(self, mode):
        #AuRED because of model name
        # try:
        #  pretrained_model_name=self.hparams.pretrained_model_name
        # except:
        pretrained_model_name=self.hparams.pretrained_model_name.split("/")[1]
        return (
            Path(self.hparams.data_dir)
            #original code
            # / f"cached_{mode}_{self.hparams.pretrained_model_name}_{self.hparams.max_seq_length}"
            #AuRED
            / f"cached_{mode}_{pretrained_model_name}_{self.hparams.max_seq_length}"
        )

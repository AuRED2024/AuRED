...
def save_to_trec_format(all_predict, trec_outpath):
    ...
    for key, values in all_predict.items():
            ...
            for value in sorted_values[:5]:
                output_rumor_id.append(key)
                output_evidence_id.append(value[1])
                output_score.append(value[4])
                output_rank.append(counter)
                counter=counter+1
    output_df['Rumor ID']=output_rumor_id
    output_df['Q0']=['Q0']*len(output_rumor_id)
    output_df['Evidence ID']=output_evidence_id
    output_df['Rank']=output_rank
    output_df['Score']=output_score
    output_df['Tag']=['TripletLoss']*len(output_rumor_id)
    print(output_df)
    output_df.to_csv(trec_outpath,sep="\t",index=False,header=False)
...
def eval_model(model, validset_reader):
    ...
    for inp_tensor, msk_tensor, seg_tensor, ids, evi_list,inp_anc_tensor, msk_anc_tensor, seg_anc_tensor in validset_reader:
        ...
        for i in range(len(dist)):
            ...
            #cosine similarity=1-pairwise distance
            all_predict[ids[i]].append(evi_list[i] + [1-dist[i]])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trec_name", type=str, required=True)                         
    ...
    trec_save_path = args.outdir + "/" + args.trec_name
    predict_dict = eval_model(model, validset_reader)
    save_to_file(predict_dict, save_path)
    save_to_trec_format(predict_dict, trec_save_path)